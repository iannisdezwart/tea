...
namespace keypress
{
	...
	uint8_t scan_compound_arrow_key()
	{
		int fourth = getch();
		if (fourth != ';') return UNKNOWN;
		...
		switch (sixth) {
			case 'A': return ARROW_UP + kind * 4;
			case 'B': return ARROW_DOWN + kind * 4;
			case 'C': return ARROW_RIGHT + kind * 4;
			case 'D': return ARROW_LEFT + kind * 4;
	...
	uint8_t scan_delete()
	{
		...
		if (fourth == '~') return DELETE;
		else if (fourth == ';') goto compound_delete;
		else return UNKNOWN;
		...
		if (sixth == '~') return SHIFT_DELETE + kind;
			return UNKNOWN;
	}
	...
	uint8_t get_key()
	{
		...
		switch (second) {
			...
			case CTRL_BACKSPACE: return CTRL_ALT_BACKSPACE;
			case ENTER: return ALT_ENTER;
			default: return UNKNOWN;
		}
		...
		switch (third)
		{
			...
			case '1': return scan_compound_arrow_key();
